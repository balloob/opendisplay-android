...
public class ProtocolTest {
    ...
    @Test
    public void testCrcDeterministic() {
        byte[] data = "Hello OpenDisplay".getBytes();
        int crc = OpenDisplayProtocol.crc16(data, 0, data.length);
        assertTrue("CRC should be non-zero", crc != 0);
        assertEquals("CRC should be deterministic",
            crc, OpenDisplayProtocol.crc16(data, 0, data.length));
    }
